...
function Board() {
  ...
  const { data, isPending, error, refetch } = useQuery({
    queryKey: ['game'], queryFn: () =>
      fetch('http://localhost:5001/game').then(res => res.json())
  });
  ...
  async function handleClick(row: number, col: number) {
    const res = await fetch("/makemove",
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ row, col }) });
    await res.json();
    refetch()
  }

  async function reset_game() {
    const res = await fetch("/resetgame", { method: "GET" });
    await res.json();
    refetch()
  }
